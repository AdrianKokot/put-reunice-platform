...
import { Page, PageService, UniversityService } from "@reunice/modules/shared/data-access";
...
export class UniversityShellComponent {
  ...
  public readonly university$ = this._universityId$.pipe(
    switchMap((id) => this._universityService.getUniversity(id).pipe(startWith(null)))
  );

  public readonly pages$ = this._universityId$.pipe(
    switchMap((id) => this._pageService.getUniversityHierarchy(id).pipe(startWith(null)))
  );
