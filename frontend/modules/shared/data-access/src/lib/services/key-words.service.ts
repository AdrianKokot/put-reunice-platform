...
export class KeyWordsService {
  ...
  getKeyWord(id: number): Observable<Keyword> {
    return this.http
      .get<Keyword>(`${this.keyWordsUrl}/${id}`);
  }

  getAllKeyWords(): Observable<Keyword[]> {
    return this.http
      .get<Keyword[]>(this.keyWordsUrl + "/all");
  }

  addKeyWord(word: string): Observable<string> {
    return this.http
      .post<string>(this.keyWordsUrl, word);
  }
  ...
  modifyKeyWordWordField(
    ...
    return this.http
      .post<void>(`${this.keyWordsUrl}/${id}`, word);
  }

  deleteKeyWord(id: number): Observable<void> {
    return this.http
      .delete<void>(`${this.keyWordsUrl}/${id}`);
  }
}
