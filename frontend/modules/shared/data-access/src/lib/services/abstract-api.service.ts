...
export abstract class AbstractApiService<T extends { id: number | string } = { id: number }, TCreatePayload = T, TUpdatePayload = T> {
  ...
  getAll(params: ApiParams<T> | ApiParams) {
    return this._http.get<T[]>(this._resourceUrl, { params: new HttpParams({ fromObject: params }) });
  }
  ...
  delete(id: T["id"]) {
    return this._http.delete(`${this._resourceUrl}/${id}`, { observe: "response" })
