...
@Component({
  selector: 'reunice-template-selector',
...
export class TemplateSelectorComponent implements OnInit {
  ...
  templateControl = new FormControl<string | Template>('', [
  ...
  ngOnInit(): void {
    ...
    this.templateControl.statusChanges.subscribe((status) => {
      if (status === 'VALID') {
        const value: Template = this.templateControl.value as Template;
  ...
  loadTemplates() {
    if (this.universityId) {
      this.templateService
        .getUniversityTemplates(this.universityId)
        .subscribe((res) => {
          this.options = [
            {
              id: 0,
