...
export class PageDetailsComponent implements OnInit, OnDestroy {
  ...
  public id = 0;
  public pageHtml: any;
  public showParentPage = true;
  public showChildPages = true;
  public xs = 600;
  ...
  ngOnInit(): void {
    ...
    this.route.paramMap.subscribe((value) => {
      ...
      this.loadPage();
      if (innerWidth < this.xs && this.sidenavExtended) this.toggleSidenav();
    });
    this.sidenavToggledSubscription = this.pageService.sidenavToggled.subscribe(
      () => {
        this.toggleSidenav();
      }
    );
  ...
  hiddenPage() {
    this.dialog
      ...
      .subscribe((res) => {
        if (res) {
          this.pageService
            .modifyPageHiddenField(this.id, !this.page.hidden)
            .subscribe(() => {
      ...
      .subscribe((res) => {
        if (res) {
          this.pageService.deletePage(this.id).subscribe({
            next: () => {
              this.dialogService.openSuccessDialog(
                this.translate.instant('DELETE_PAGE_CONFIRMATION')
              );
  ...
  loadHierarchy() {
    ...
    const uniId = this.page.university.id;
    this.pageService.getUniversityHierarchy(uniId).subscribe({
      next: (res) => {
  ...
  onResize(event: UIEvent) {
    ...
    if (event.target.innerWidth >= 1740) this.sidenav.mode = 'over';
    ...
    if (this.sidenavExtended) {
      ...
      else this.sidenavWidth = this.toPixels(event.target.innerWidth);
      if (event.target.innerWidth < this.xs) this.sidenav.open!();
    } else {
      this.sidenavWidth = this.toPixels(50);
      if (event.target.innerWidth < this.xs) this.sidenav.close!();
    }
  }
