...
@Component({
  selector: 'reunice-dialog-page-edit-base',
...
export class DialogPageEditBasicComponent implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<DialogPageEditBasicComponent>,
  ...
  changePageInformation() {
    if (
      ...
      this.titleValid.status == 'VALID'
    ) {
      const pageData: PageUpdateForm = {
        title: this.title,
      ...
      this.pageService.editPage(this.pageId, pageData).subscribe({
        next: () => {
          ...
          this.dialogService.openSuccessDialog(
            this.translate.instant('INFORMATION_CHANGED')
          );
        },
        error: () => {
          this.close();
