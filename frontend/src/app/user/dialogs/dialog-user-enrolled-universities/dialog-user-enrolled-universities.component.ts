...
@Component({
  selector: 'reunice-dialog-user-enrolled-universities',
...
export class DialogUserEnrolledUniversitiesComponent implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<DialogUserEnrolledUniversitiesComponent>,
    @Inject(MAT_DIALOG_DATA)
    public data: { user: { id: number; enrolledUniversities: University[] } },
    ...
    this.filteredUniversities = this.universityControl.valueChanges.pipe(
      ...
      map((value) => {
        ...
        return name
          ...
          : this.availableUniversities.slice();
      })
    );
  }
    ...
    if (this.userService.loggedUser) {
      ...
      if (loggedUser.accountType === 'ADMIN') {
        this.universityService.getUniversities().subscribe((res) => {
          ...
          this.availableUniversities = res.filter(
            (u) => !this.selectedUniversities.map((x) => x.id).includes(u.id)
          );
          this.universityControl.setValue('');
        });
      } else {
        this.selectedUniversities = [...this.data.user.enrolledUniversities];
        this.availableUniversities =
          this.userService.loggedUser?.enrolledUniversities.filter(
            (u) => !this.selectedUniversities.map((x) => x.id).includes(u.id)
          ) ?? [];
        this.universityControl.setValue('');
      }
  ...
  private _filter(name: string): University[] {
    ...
    return this.availableUniversities.filter((option) =>
        ...
        if (value && this.data.user) {
          ...
          this.userService
            .updateUserEnrolledUniversities(
              this.data.user.id,
              this.selectedUniversities.map((u) => u.id)
            )
            .subscribe({
              next: (user) => {
                this.dialogService.openSuccessDialog(
                  this.translate.instant('USER_ENROLLED')
                );
  ...
  isChipDisabled(university: University): boolean {
    return (
      this.userService.loggedUser?.accountType === 'MODERATOR' &&
      !this.userService.loggedUser.enrolledUniversities
        .map((u) => u.id)
