...
import { Backup } from '@reunice/modules/shared/data-access';
import { DialogService } from '../../assets/service/dialog.service';
import { saveAs } from 'file-saver';
import { TranslateService } from '@ngx-translate/core';

@Component({
  selector: 'reunice-backup',
        ...
        if (value) {
          ...
          this.backupService.createBackup().subscribe(() => {
            this.dialogService.openSuccessDialog(
              this.translateService.instant('BACKUP.CREATE_SUCCESS')
            );
      ...
      .subscribe((value) => {
        if (value) {
          this.backupService.deleteBackup(backup.name).subscribe(() => {
            this.dialogService.openSuccessDialog(
              this.translateService.instant('BACKUP.DELETE_SUCCESS')
            );
            this.loadBackups();
          });
        }
  ...
  updateTimeSinceLastBackup() {
    if (this.backups.length > 0 && this.backups[0].name) {
      const milliseconds: number =
        Date.now() - Number.parseInt(this.backups[0].name);
      ...
      this.timeSinceLastBackup =
        this.translateService.instant('BACKUP.NO_BACKUPS');
    }
  }
}
